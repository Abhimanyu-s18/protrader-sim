...
adminRouter.get('/users', async (req, res, next) => {
  try {
    const { q, kyc_status, account_status, cursor, limit = '50' } = req.query as Record<string, string>
    const take = Math.min(parseInt(limit, 10), 200)
    const users = await prisma.user.findMany({
      where: {
        ...(q ? { OR: [{ email: { contains: q, mode: 'insensitive' } }, { fullName: { contains: q, mode: 'insensitive' } }, { accountNumber: { contains: q } }] } : {}),
        ...(kyc_status ? { kycStatus: kyc_status as never } : {}),
        ...(account_status ? { accountStatus: account_status as never } : {}),
        ...(cursor ? { id: { lt: BigInt(cursor) } } : {}),
      },
      select: { id: true, accountNumber: true, leadId: true, email: true, fullName: true, phone: true, country: true, kycStatus: true, accountStatus: true, emailVerified: true, agentId: true, createdAt: true },
      orderBy: { createdAt: 'desc' },
      take: take + 1,
    })
    const hasMore = users.length > take
    res.json(serializeBigInt({ data: hasMore ? users.slice(0, take) : users, next_cursor: hasMore ? users[take - 1]?.id.toString() : null, has_more: hasMore }))
  } catch (err) { next(err) }
})
...
adminRouter.get('/users/:id', async (req, res, next) => {
  try {
    const user = await prisma.user.findUnique({
      ...
      include: { kycDocuments: true, deposits: { take: 10, orderBy: { createdAt: 'desc' } }, withdrawals: { take: 10, orderBy: { createdAt: 'desc' } } },
    })
...
adminRouter.put('/users/:id/status', async (req, res, next) => {
  try {
    ...
    const user = await prisma.user.update({ where: { id: BigInt(req.params['id']!) }, data: { accountStatus: status }, select: { id: true, accountStatus: true } })
    res.json(serializeBigInt(user))
  } catch (err) { next(err) }
})
...
adminRouter.post('/users/:id/adjustment', async (req, res, next) => {
  try {
    const { amount_cents, description } = z.object({ amount_cents: z.number().int(), description: z.string().min(5) }).parse(req.body)
    const userId = BigInt(req.params['id']!)
    const [bal] = await prisma.$queryRaw<[{ balance_cents: bigint }]>`SELECT get_user_balance(${userId}) AS balance_cents`
    await prisma.ledgerTransaction.create({
      data: { userId, transactionType: 'MANUAL_ADJUSTMENT', amountCents: BigInt(amount_cents), balanceAfterCents: (bal?.balance_cents ?? 0n) + BigInt(amount_cents), description, createdBy: BigInt(req.user!.user_id) },
    })
...
adminRouter.get('/kyc', async (req, res, next) => {
  try {
    ...
    res.json(serializeBigInt({ data: hasMore ? docs.slice(0, take) : docs, next_cursor: hasMore ? docs[take - 1]?.id.toString() : null, has_more: hasMore }))
  } catch (err) { next(err) }
})

adminRouter.put('/kyc/:doc_id', async (req, res, next) => {
  try {
    const { status, rejection_reason } = z.object({
    ...
    const doc = await prisma.kycDocument.update({
      ...
      data: { status: status as never, rejectionReason: rejection_reason ?? null, reviewedBy: BigInt(req.user!.user_id), reviewedAt: new Date() },
    ...
    if (status === 'APPROVED') {
      ...
      const idApproved = await prisma.kycDocument.count({ where: { userId, documentCategory: 'IDENTITY', status: 'APPROVED' } })
      const addrApproved = await prisma.kycDocument.count({ where: { userId, documentCategory: 'ADDRESS', status: 'APPROVED' } })
    ...
    const signedUrl = await getSignedUrl(r2, new GetObjectCommand({ Bucket: process.env['CLOUDFLARE_R2_BUCKET_NAME'] ?? 'protrader-kyc-docs', Key: doc.r2Key }), { expiresIn: 900 })
...
adminRouter.get('/deposits', async (req, res, next) => {
  try {
    ...
    const deposits = await prisma.deposit.findMany({
      where: { ...(status ? { status: status as never } : {}), ...(cursor ? { id: { lt: BigInt(cursor) } } : {}) },
    ...
    res.json(serializeBigInt({ data: hasMore ? deposits.slice(0, take) : deposits, next_cursor: hasMore ? deposits[take - 1]?.id.toString() : null, has_more: hasMore }))
  } catch (err) { next(err) }
})
...
adminRouter.get('/withdrawals', async (req, res, next) => {
  try {
    ...
    res.json(serializeBigInt({ data: hasMore ? items.slice(0, take) : items, next_cursor: hasMore ? items[take - 1]?.id.toString() : null, has_more: hasMore }))
  } catch (err) { next(err) }
})
