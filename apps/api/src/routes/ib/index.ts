...
ibRouter.get('/traders', async (req, res, next) => {
  try {
    ...
    const traders = await prisma.user.findMany({
      where: { agentId },
      select: { id: true, fullName: true, email: true, accountNumber: true, kycStatus: true, accountStatus: true, createdAt: true },
...
ibRouter.get('/commissions', async (req, res, next) => {
  try {
    ...
    const commissions = await prisma.ibCommission.findMany({
      where: { agentId, ...(status ? { status: status as never } : {}), ...(cursor ? { id: { lt: BigInt(cursor) } } : {}) },
      include: {
        trader: { select: { fullName: true, accountNumber: true } },
        trade: { select: { direction: true, units: true, openAt: true, instrument: { select: { symbol: true } } } },
      },
      orderBy: { createdAt: 'desc' },
      take: take + 1,
    })
...
ibRouter.get('/commissions/summary', async (req, res, next) => {
  try {
    ...
    const [total, pending, paid] = await Promise.all([
      ...
      prisma.ibCommission.aggregate({ where: { agentId, status: 'PENDING' }, _sum: { amountCents: true } }),
      prisma.ibCommission.aggregate({ where: { agentId, status: 'PAID' }, _sum: { amountCents: true } }),
    ])
    ...
    res.json(serializeBigInt({
      total_cents: totalCents, total_formatted: formatCents(totalCents),
      pending_cents: pendingCents, pending_formatted: formatCents(pendingCents),
      paid_cents: paidCents, paid_formatted: formatCents(paidCents),
    }))
  } catch (err) { next(err) }
})
...
ibRouter.get('/network-stats', async (req, res, next) => {
  try {
    ...
    const [traderCount, activeTraders, totalVolume, commissionSummary] = await Promise.all([
      ...
      prisma.trade.aggregate({ where: { userId: { in: traderIds }, status: 'CLOSED' }, _sum: { units: true } }),
      prisma.ibCommission.aggregate({ where: { agentId, status: 'PENDING' }, _sum: { amountCents: true } }),
    ])
