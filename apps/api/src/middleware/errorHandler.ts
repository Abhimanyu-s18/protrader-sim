...
export const Errors = {
  ...
  kycRequired: () => new AppError('KYC_REQUIRED', 'KYC verification must be approved before trading.', 403),
  notFound: (resource: string) => new AppError('NOT_FOUND', `${resource} not found.`, 404),
  insufficientFunds: () => new AppError('INSUFFICIENT_FUNDS', 'Insufficient available balance.', 409),
  insufficientMargin: () => new AppError('INSUFFICIENT_MARGIN', 'Insufficient available margin for this trade.', 409),
  marketClosed: (symbol: string) => new AppError('MARKET_CLOSED', `${symbol} is not available for trading at this time.`, 409),
  ...
  validation: (details: Record<string, unknown>) => new AppError('VALIDATION_ERROR', 'Request validation failed.', 400, details),
