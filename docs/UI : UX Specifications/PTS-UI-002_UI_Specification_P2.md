...
_Accessibility: colors must never be the sole indicator of category or direction. Always pair with text label or icon. Test with color-blindness simulator before Sprint 8 sign-off._
...
_All animations wrapped in `@media (prefers-reduced-motion: no-preference) { }`. Only animate `transform` and `opacity` — never animate height, width, or background-color directly._
...
_Mobile: increase all base font sizes by 2px (14px body → 16px, 12px caption → 14px)._
...
_ProTraderSim — PTS-UI-002 — UI Functional Specification Part 2 — v1.0 — March 2026_
